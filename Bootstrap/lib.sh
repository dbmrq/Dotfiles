...
# Ensure $HOME/.ssh and $HOME/.config/{opencode,zed} are real directories before
# stowing.
#
...
# tree. For SSH it would put private keys, agent sockets, and known_hosts inside
# the repo. Pre-creating the real directories forces Stow to link each managed
# file individually, keeping runtime state outside the repo.
#
# Idempotent: no-op when the directory already exists (real or symlink). Never
# deletes anything.
ensure_managed_config_dirs() {
    local dir
    for dir in "$HOME/.ssh" "$HOME/.config/opencode" "$HOME/.config/zed"; do
        if [[ -d "$dir" && ! -L "$dir" ]]; then
